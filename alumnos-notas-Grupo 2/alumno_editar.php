...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!$errores) {
    $upd = $pdo->prepare("UPDATE alumno SET nombre = ?, apellido = ?, correo = ? WHERE id = ?");
    try {
      $upd->execute([$nombre, $apellido, $correo, $id]);
      header("Location: index.php");
      exit;
    } catch (PDOException $e) {
      $errores[] = "No se pudo actualizar. Puede que el correo ya exista.";
    }
  }
}
